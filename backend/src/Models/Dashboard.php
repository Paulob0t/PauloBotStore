    ...
    public function getInventoryMetrics(): array
    {
        ...
        if ($this->db && !$this->db->connect_error) {
            $sqlProd = "SELECT
                ...
                SUM(CASE WHEN stock <= 5 AND (activo = 1 OR activo IS NULL) THEN 1 ELSE 0 END) AS stock_bajo,
    ...
    public function getTopProducts(int $limit = 5): array
    {
        ...
        if ($this->db && !$this->db->connect_error) {
            $sqlTop = "SELECT p.nombre_producto, SUM(vd.cantidad) AS unidades, COALESCE(SUM(vd.total), 0) AS ingresos
                       ...
                       INNER JOIN ventas_comanda vc ON vc.id_comanda = vd.id_comanda
            ...
            if ($stmt = $this->db->prepare($sqlUlt)) {
                ...
                while ($r = $res->fetch_assoc()) {
                    $ultimasVentas[] = [
                        ...
                        'tipo_pago' => $r['tipo_pago'] ? (string)$r['tipo_pago'] : null
                    ];
                }
                $stmt->close();
            }
