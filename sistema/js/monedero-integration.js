...
const MonederoIntegration = {
        ...
        const displayHTML = `
            <div id="monedero-display" class="wallet-widget collapsed">
                ...
                <div class="wallet-tab" onclick="MonederoIntegration.toggleWidget()" title="Ver mi saldo">
                    ...
                    <div class="wallet-tab-label">MI SALDO</div>
                </div>
                
                <!-- Contenido expandido -->
                <div class="wallet-content">
                    <div class="wallet-header">
                        <h3 class="wallet-title">SALDO MONEDERO</h3>
        ...
        fetch('monedero_api.php?action=get_saldo&_=' + Date.now())
            ...
            .then(data => {
                if (data.success && data.saldo !== null && data.saldo !== undefined) {
                    const nuevoSaldo = parseFloat(data.saldo);
    ...
    agregarBotonPagoMonedas: function() {
        ...
        const checkAndModify = () => {
            ...
            if (btnEfectivo && !btnEfectivo.dataset.monederoActivado) {
                ...
                // Modificar el evento click para usar monedas si hay saldo
                btnEfectivo.addEventListener('click', (e) => {
                    if (this.saldoActual > 0) {
                        e.preventDefault();
                        e.stopPropagation();
                        this.procesarPagoConMonedas();
                    }
                    // Si no hay saldo, deja que el flujo normal continue
                });
    ...
    procesarPagoConMonedas: function() {
        ...
        if (this.saldoActual < totalCarrito) {
            Swal.fire({
                ...
                html: `
                    <p>Total a pagar: <strong>$${totalCarrito.toFixed(2)}</strong></p>
                    <p>Saldo disponible: <strong>$${this.saldoActual.toFixed(2)}</strong></p>
                    <p>Faltan: <strong>$${(totalCarrito - this.saldoActual).toFixed(2)}</strong></p>
                `,
                confirmButtonText: 'Insertar más monedas'
            });
            return;
        }
    ...
    finalizarVenta: function(total) {
        ...
        .then(data => {
            if (data.success) {
                ...
                // ✅ SEGUNDO: Resetear saldo SOLO después de confirmar éxito
                this.resetearSaldo();
                
                // 💰 TERCERO: Dispensar cambio si hay
     ...
     */
    dispensarCambioFisico: function(monto) {
        return new Promise((resolve, reject) => {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    console.log('📊 Desglose:', data.desglose);
                    console.log('🪙 Total monedas:', data.total_monedas);
                    console.log('📡 Comandos enviados:', data.comandos);
                    
                    // Cerrar loading
                    Swal.close();
                    
                    // Crear desglose HTML con comandos enviados
                    const desgloseHTML = Object.entries(data.desglose || {})
                        .map(([denom, cant]) => {
                            const hex = parseInt(denom).toString(16).toUpperCase();
                            return `<div class="d-flex justify-content-between align-items-center mb-2 p-2 bg-light rounded">
                                <span class="fw-bold">💰 $${denom}</span>
                                <span class="badge bg-primary">${cant} × moneda(s)</span>
                                <small class="text-muted">CMD: INT000${hex}003</small>
                            </div>`;
                        })
                        .join('');
                        ...
                        html: `
                            <div class="text-start">
                                <div class="alert alert-success mb-3">
                                    <strong class="fs-5">✅ Total Dispensado: $${monto.toFixed(2)}</strong>
                                </div>
                                <div class="mt-3">
                                    <h6 class="mb-3"><i class="bi bi-cash-stack"></i> Desglose de Monedas:</h6>
                                    ${desgloseHTML}
                                </div>
                                <div class="mt-3 p-2 bg-info bg-opacity-10 rounded">
                                    <small class="text-muted d-flex justify-content-between">
                                        <span>🪙 Total monedas:</span>
                                        <strong>${data.total_monedas}</strong>
                                    </small>
                                </div>
                            </div>
                        `,
                ...
                <div class="text-start">
                    <div class="alert alert-success">
                        ...
                        <div class="d-flex justify-content-between mb-2 pb-2 border-bottom">
                            <span>💵 Cambio:</span>
                            <strong class="text-warning">$${cambio.toFixed(2)}</strong>
                        </div>
                        <div class="d-flex justify-content-between mt-2">
                            <span>📄 Folio:</span>
                            <strong>${folio || 'N/A'}</strong>
                        </div>
                    </div>
                    ${cambio > 0 ? '<p class="text-center text-success"><i class="bi bi-check-circle-fill"></i> Cambio dispensado</p>' : ''}
                </div>
            `,
