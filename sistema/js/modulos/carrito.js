    // ✅ Manejar localStorage bloqueado por Tracking Prevention
    let cartItems = [];
    let serviceItems = [];
    
    // 🛡️ Helper para acceso seguro a localStorage
    const safeLocalStorage = {
        getItem: (key) => {
            try {
                return localStorage.getItem(key);
            } catch (error) {
                console.warn(`⚠️ No se pudo leer ${key} de localStorage:`, error);
                return null;
            }
        },
        setItem: (key, value) => {
            try {
                ...
                console.warn(`⚠️ No se pudo guardar ${key} en localStorage:`, error);
                return false;
            }
        },
        removeItem: (key) => {
            try {
                ...
                console.warn(`⚠️ No se pudo eliminar ${key} de localStorage:`, error);
                return false;
            }
        }
    };
    ...
    try {
        cartItems = JSON.parse(safeLocalStorage.getItem('cart')) || [];
        serviceItems = JSON.parse(safeLocalStorage.getItem('serviceCart')) || [];
    } catch (error) {
        console.warn('⚠️ Error al parsear datos del carrito:', error);
        cartItems = [];
        serviceItems = [];
    }
    ...
    class Cart {
        ...
        // ✅ Getter para acceder a los items del carrito
        static get items() {
            return cartItems || [];
        }
        
        // ✅ Getter para acceder a los items de servicios
        static get serviceItems() {
            return serviceItems || [];
        }
        ...
        static loadFromStorage() {
            // 🔥 Recargar datos desde localStorage con manejo de errores
            try {
                cartItems = JSON.parse(safeLocalStorage.getItem('cart')) || [];
                serviceItems = JSON.parse(safeLocalStorage.getItem('serviceCart')) || [];
                console.log('📦 Carrito recargado:', cartItems.length, 'productos');
            } catch (error) {
                console.warn('⚠️ Error al parsear carrito:', error);
                cartItems = [];
                serviceItems = [];
            }
        }
        ...
        static saveToStorage() {
            ...
            try {
                ...
                safeLocalStorage.setItem('serviceCart', JSON.stringify(serviceItems));
                return true;
            } catch (error) {
                console.warn('⚠️ Error al guardar carrito:', error);
                return false;
            }
        }
        ...
        static async procesarVenta(metodoPago, tipoPago, tipoTarjeta = 0) {
            ...
            try {
                ...
                // � RESETEAR MONEDERO (hardware + UI)
                if (typeof window.resetCashDisplay === 'function') {
                    window.resetCashDisplay();
                }
                
                // Resetear saldo físico del monedero
                try {
                    await fetch('monedero_api.php?action=resetear_saldo&_=' + Date.now());
                    console.log('✅ Monedero reseteado');
                } catch (err) {
                    console.warn('⚠️ No se pudo resetear el monedero:', err);
                }
                
                // �💾 GUARDAR datos del ticket en sessionStorage para pago_aprobado.php
                ...
                if (ventaResult.ticket_data) {
                    if (typeof PrintTicket !== 'undefined' && typeof qz !== 'undefined') {
                        try {
                            ...
                            // Esperar máximo 5 segundos para la impresión
                            const timeoutPromise = new Promise((_, reject) => 
                                setTimeout(() => reject(new Error('Timeout impresión')), 5000)
                            );
                            ...
                            // Esperar 500ms adicionales para asegurar que terminó
                            await new Promise(resolve => setTimeout(resolve, 500));
                            
                        } catch (printError) {
                            // Continuar aunque falle la impresión (timeout o error)
                        }
                    }
                }
                
                // Redireccionar a página de aprobación DESPUÉS de imprimir
        ...
        static async handleCashPayment() {
            ...
            // 💰 OBTENER DINERO INSERTADO AUTOMÁTICAMENTE (sin preguntar)
            let amountReceived = 0;
            
            if (typeof window.getMonederoSaldo === 'function') {
                amountReceived = window.getMonederoSaldo();
                console.log('💰 Dinero detectado automáticamente:', amountReceived);
            } else {
                console.warn('⚠️ window.getMonederoSaldo no disponible');
            }
                ...
                await Swal.fire({
                    ...
                    html: `
                        <div class="alert alert-danger">
                            <h4 class="mb-3">No hay suficiente dinero insertado</h4>
                            <div class="text-start">
                                <p><strong>Total a pagar:</strong> <span class="text-danger fs-5">$${total.toFixed(2)}</span></p>
                                <p><strong>Dinero insertado:</strong> <span class="text-success fs-5">$${amountReceived.toFixed(2)}</span></p>
                                <hr>
                                <p class="mb-0"><strong>Faltante:</strong> <span class="text-warning fs-4">$${faltante.toFixed(2)}</span></p>
                            </div>
                        </div>
                        <div class="alert alert-info mt-3 mb-0">
                            <i class="bi bi-info-circle me-2"></i>
                            <strong>Por favor, inserta $${faltante.toFixed(2)} más</strong>
                        </div>
                    `,
                    confirmButtonText: 'Entendido',
            ...
            // ✅ HAY SUFICIENTE DINERO - Proceder con el pago
            this.handleCashPaymentWithAmount(amountReceived);
        }
        ...
        static async handleCashPaymentWithAmount(amountReceived) {
                    ...
                    <div class="text-start">
                        <div class="card border-0 shadow-sm" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);">
                            <div class="card-body p-4">
                                <div class="d-flex justify-content-between mb-3 pb-3 border-bottom border-white">
                                    ...
                                    <strong class="text-white fs-5">$${total.toFixed(2)}</strong>
                                </div>
                                
                                <div class="d-flex justify-content-between mb-3 pb-3 border-bottom border-white">
                                    <span class="text-white">
                                        <i class="bi bi-cash-stack me-2"></i>Dinero recibido:
                                    </span>
                                    <strong class="text-white fs-5">$${amountReceived.toFixed(2)}</strong>
                                </div>
                                
                                <div class="bg-white rounded-3 p-3 text-center">
                                    <small class="text-muted d-block mb-1">Cambio a devolver</small>
                                    <strong class="text-warning d-block" style="font-size: 2.5rem; line-height: 1;">$${change.toFixed(2)}</strong>
                                </div>
                            </div>
                        </div>
                        ...
                        ${change > 0 ? `
                            <div class="alert alert-warning mt-3 mb-0">
                                <i class="bi bi-exclamation-triangle-fill me-2"></i>
                                <strong>¡Devuelve $${change.toFixed(2)} al cliente!</strong>
                            </div>
                        ` : `
                            <div class="alert alert-success mt-3 mb-0">
                                <i class="bi bi-check-circle-fill me-2"></i>
                                <strong>✓ Pago exacto - Sin cambio</strong>
                            </div>
                        `}
                    </div>
                `,
            ...
            if (result.isConfirmed) {
                // ✅ PRIMERO DISPENSAR CAMBIO (si hay), LUEGO PROCESAR VENTA
                if (change > 0) {
                    // Dispensar cambio físicamente ANTES de guardar en BD
                    await this.dispensarYProcesarVenta(change);
                } else {
                    // No hay cambio, procesar directamente
                    await this.procesarVenta('Efectivo', 2, 0);
                }
            }
        }
        ...
        static async dispensarYProcesarVenta(cambio) {
            ...
            try {
                if (typeof MonederoIntegration !== 'undefined' && typeof MonederoIntegration.dispensarCambioFisico === 'function') {
                    ...
                    // Dispensar y esperar resultado
                    const resultadoDispensado = await MonederoIntegration.dispensarCambioFisico(cambio);
                    
                    if (resultadoDispensado && resultadoDispensado.success) {
                        cambioDispensado = true;
                        console.log('✅ Cambio dispensado exitosamente');
                    } else {
                        throw new Error('Dispensado falló: ' + (resultadoDispensado?.mensaje || 'Error desconocido'));
                    }
                ...
                console.error('❌ Error al dispensar cambio:', error);
                Swal.close();
                
                // Preguntar al operador si entregó el cambio manualmente
                const confirmManual = await Swal.fire({
                    icon: 'warning',
                    title: '⚠️ Dispensador No Disponible',
                    html: `
                        <div class="alert alert-danger">
                            <h3 class="mb-3">$${cambio.toFixed(2)}</h3>
                            <p class="mb-2"><strong>El dispensador automático falló</strong></p>
                            <p class="text-muted small">${error.message}</p>
                        </div>
                        <div class="alert alert-warning">
                            <p class="mb-0">
                                <i class="bi bi-exclamation-triangle-fill me-2"></i>
                                <strong>¿Entregaste el cambio MANUALMENTE al cliente?</strong>
                            </p>
                        </div>
                    `,
                    showCancelButton: true,
                    confirmButtonText: '✅ Sí, entregué $' + cambio.toFixed(2),
                    cancelButtonText: '❌ No, cancelar venta',
                    confirmButtonColor: '#28a745',
                ...
                if (!confirmManual.isConfirmed) {
                    // Operador NO entregó cambio, cancelar venta
                    Swal.fire({
                        icon: 'error',
                        title: 'Venta Cancelada',
                        text: 'La venta fue cancelada porque no se entregó el cambio',
                        confirmButtonColor: '#dc3545'
                    });
                    return; // ❌ SALIR SIN PROCESAR
                }
                
                cambioDispensado = true; // Marcar como entregado manualmente
            }
            
            // PASO 2: Procesar venta (SOLO si el cambio fue dispensado/entregado)
            if (cambioDispensado) {
                Swal.fire({
        ...
        static addItem(product) {
            ...
            if (existingItem) {
                ...
                // Convertir descuento de porcentaje a pesos para cálculos internos
                const precioBase = parseFloat(product.precio) || 0;
                const descPct = parseFloat(product.descuento) || 0;
                const descPesos = precioBase * (descPct / 100);
        ...
        static init() {
            ...
            this.handleCartClick = (e) => {
                ...
                if (e.target.closest('.add-to-cart-btn')) {
                    ...
                    const productId = btn.dataset.productId;
                    const product = (window.products && Array.isArray(window.products))
                        ? window.products.find(p => p.id_producto === productId)
                        : null;
