...
export class TelmexService {
    static async init() {
        try {
            ...
            console.error('Error al inicializar servicios Telmex:', error);
            Swal.fire({
                title: 'Error',
                text: 'No se pudieron cargar los servicios de Telmex',
                icon: 'error'
            });
        }
    }
    ...
    static async loadServices() {
        try {
            const response = await fetch('telmex_telefonia_functional.php');
            ...
            telmexServices = [
                {
                    sku: 'TELMEXTEL',
                    formatted_name: 'Telefonía Telmex',
                    description: 'Pago de servicio telefónico Telmex'
                },
                {
                    sku: 'TELMEXINT',
                    formatted_name: 'Internet Telmex',
                    description: 'Pago de servicio de internet Telmex'
                }
            ];
            ...
            Swal.fire({
                title: 'Error',
                text: 'No se pudo cargar el servicio de Telmex. Por favor intenta más tarde.',
                icon: 'error'
            });
            throw error;
        }
    ...
    static renderServicesContainer() {
        const container = document.getElementById('products-container');
        if (!container || container.querySelector('.telmex-service-card')) return;

        const serviceCard = document.createElement('div');
        serviceCard.className = 'col-lg-6 col-md-6 col-12 telmex-service-card';
        serviceCard.innerHTML = `
            <div class="card product-card" style="margin: 10px; border-radius: 15px; overflow: hidden; box-shadow: 0 4px 15px rgba(0,0,0,0.1); height: 700px;">
                <div style="position: relative;">
                    <img src="https://upload.wikimedia.org/wikipedia/commons/thumb/7/75/Telmex_Logo.svg/1920px-Telmex_Logo.svg.png" 
                         alt="Telmex" 
                ...
                <div class="card-body" style="padding: 20px; display: flex; flex-direction: column; gap: 15px;">
                    ...
                    <div class="service-list" style="max-height: 200px; overflow-y: auto; margin-bottom: 15px;">
                        ${this.renderServiceList(telmexServices)}
                    </div>
                    
                    <div class="input-section">
                        <div class="number-input">
                            <input type="text" id="telmex-service-number" placeholder="Número telefónico" 
                                   style="width: 100%; padding: 10px; border: 2px solid #f1f1f1; border-radius: 5px; margin-bottom: 10px;"
                                   pattern="[0-9]*" inputmode="numeric" ${telmexServices.length === 0 ? 'disabled' : ''}>
                            <div id="telmex-number-error" style="color: #ff6b6b; font-size: 0.8rem; margin-top: 5px; display: none;">
                                Número telefónico inválido (10 dígitos)
                            </div>
                        </div>
                        <div class="amount-input" style="margin-top: 10px;">
                            <input type="number" id="telmex-amount" placeholder="Monto a pagar" 
                                   style="width: 100%; padding: 10px; border: 2px solid #f1f1f1; border-radius: 5px;"
                                   min="0" step="0.01" ${telmexServices.length === 0 ? 'disabled' : ''}>
                            <div id="telmex-amount-error" style="color: #ff6b6b; font-size: 0.8rem; margin-top: 5px; display: none;">
                    ...
                    <button class="btn" id="process-telmex" style="width: 100%; background: ${telmexServices.length === 0 ? '#999999' : '#cccccc'}; color: white; padding: 12px; border: none; border-radius: 5px; font-weight: 600; margin-top: 15px; cursor: ${telmexServices.length === 0 ? 'not-allowed' : 'pointer'};" ${telmexServices.length === 0 ? 'disabled' : ''}>
                        ${telmexServices.length === 0 ? 'Servicio no disponible' : 'Procesar Pago'}
                    </button>
                </div>
            </div>
        `;
    ...
    static validateFields() {
        if (telmexServices.length === 0) return;

        const serviceNumberInput = document.getElementById('telmex-service-number');
        const amountInput = document.getElementById('telmex-amount');
        const serviceNumberError = document.getElementById('telmex-number-error');
        const amountError = document.getElementById('telmex-amount-error');
        const processButton = document.getElementById('process-telmex');
        const selectedService = document.querySelector('.telmex-service-card .service-item.selected');
        ...
        const isValidServiceNumber = /^\d{10}$/.test(serviceNumber);
    ...
    static setupEventListeners() {
        ...
        document.querySelectorAll('.telmex-service-card .service-item').forEach(item => {
            item.addEventListener('click', () => {
                document.querySelectorAll('.telmex-service-card .service-item').forEach(i => {
    ...
    static async processPayment() {
        if (!telmexServices.length) {
            Swal.fire('Error', 'El servicio de Telmex no está disponible en este momento', 'error');
            return;
        }
        ...
        const serviceNumber = document.getElementById('telmex-service-number')?.value.trim();
        const amount = parseFloat(document.getElementById('telmex-amount')?.value);
        ...
        Swal.fire({
            ...
            html: `
                <div style="text-align: left; padding: 10px;">
                    ...
                    <p><strong>Número:</strong> ${serviceNumber}</p>
                ...
                .then(data => {
                    ...
                    if (data.success) {
                        Swal.fire({
                            ...
                            document.getElementById('telmex-service-number').value = '';
                            document.getElementById('telmex-amount').value = '';
                            document.querySelectorAll('.telmex-service-card .service-item').forEach(i => {
                                i.classList.remove('selected');
                                i.style.backgroundColor = '';
                            });
                            document.getElementById('process-telmex').disabled = true;
                        });
                    } else {
                        ...
                        if (data.httpCode) {
                            errorMessage += ` (Código: ${data.httpCode})`;
                        }
...
document.addEventListener('DOMContentLoaded', () => {
    setTimeout(() => TelmexService.init(), 100);
});
