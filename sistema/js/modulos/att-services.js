...
export class AttServices {
    static async init() {
        try {
            ...
            Swal.fire('Error', 'No se pudieron cargar los servicios de AT&T', 'error');
        }
    }
    ...
    static async obtenerSaldoMonedero() {
        try {
            ...
            if (data.success) {
                ...
                attServices.monederoSaldo = nuevoSaldo;
                this.actualizarBadgeSaldo();
            }
        } catch (error) {
            attServices.monederoSaldo = 0;
            this.actualizarBadgeSaldo();
        }
    }

    static actualizarBadgeSaldo() {
        const badgeElement = document.getElementById('att-saldo');
        if (badgeElement) {
            const saldoAnterior = parseFloat(badgeElement.textContent.replace(/[^0-9.]/g, '')) || 0;
            const saldoNuevo = attServices.monederoSaldo;
            
            // Animación de conteo progresivo solo para el badge de AT&T
        ...
        if (typeof MonederoIntegration !== 'undefined') {
            ...
            if (MonederoIntegration.saldoActual !== attServices.monederoSaldo) {
                MonederoIntegration.saldoActual = attServices.monederoSaldo;
            }
        }
    }
    ...
    static async descontarSaldo(monto) {
        try {
            ...
            const nuevoSaldo = Math.max(0, attServices.monederoSaldo - monto);
    ...
    static iniciarActualizacionSaldo() {
        if (attServices.saldoInterval) {
            clearInterval(attServices.saldoInterval);
        }
        
        // Sincronizar con MonederoIntegration en lugar de hacer polling independiente
        attServices.saldoInterval = setInterval(() => {
            ...
            if (typeof MonederoIntegration !== 'undefined') {
                ...
                if (nuevoSaldo !== attServices.monederoSaldo) {
                    attServices.monederoSaldo = nuevoSaldo;
                    this.actualizarBadgeSaldo();
    ...
    static detenerActualizacionSaldo() {
        if (attServices.saldoInterval) {
            clearInterval(attServices.saldoInterval);
            attServices.saldoInterval = null;
        }
    }
    ...
    static mostrarSaldoInsuficiente(price) {
        ...
        const faltante = total - attServices.monederoSaldo;
        Swal.fire({
            ...
            html: `
                <div style="text-align: center; padding: 1rem;">
                    ...
                    <p style="font-size: 1.1rem; margin-bottom: 0.5rem;">
                        Monto recarga: <strong style="color: #009FDB;">$${price.toFixed(2)}</strong>
                    </p>
                    ...
                    <p style="font-size: 1rem; color: #6b7280; margin-bottom: 1rem;">
                        Tu saldo actual: <strong style="color: #7c3aed;">$${attServices.monederoSaldo.toFixed(2)}</strong>
                    </p>
    ...
    static async mostrarRecargaExitosa(name, phone, price, codeDesc = null) {
        ...
        const saldoInicial = attServices.monederoSaldo + totalCobrado; // Saldo antes de descontar
        ...
        await Swal.fire({
            ...
            html: `
                <div style="text-align: left; padding: 1.5rem; background: linear-gradient(135deg, #f0fdf4 0%, #dcfce7 100%); border-radius: 12px; border: 2px solid #86efac;">
                    ...
                    <p style="margin: 0.75rem 0; font-size: 1.05rem;"><strong>📞 Número:</strong> <span style="color: #009FDB;">${phone}</span></p>
    ...
    static hapticFeedback(type = 'light') {
        if (!attServices.hapticEnabled) return;
    ...
    static showConfetti() {
        const colors = ['#009FDB', '#0066cc', '#00d4ff', '#10b981', '#f59e0b'];
    ...
    static async loadTAE(phone = '') {
        attServices.loadingServices = true;
        ...
        try {
            if (window.ProntiServices) {
                ...
                const raw = window.ProntiServices.getProducts('nextel');
                const recargas = (raw || []).filter(s => !/paquete|internet|saldo|telefonia/i.test(s.name || ''));
                if (recargas.length) {
                    attServices.tae = recargas;
                    return;
                }
            }
            attServices.tae = TAE_AMOUNTS.map(item => ({
                sku: '',
                formatted_name: item.label,
                description: 'Recarga AT&T',
    ...
    static async loadPostpago(phone = '') {
        try {
            if (window.ProntiServices) {
                await window.ProntiServices.load();
                const raw = window.ProntiServices.getProducts('nextel');
                attServices.postpago = raw.filter(p => /telefonia/i.test(p.name || ''));
                if (attServices.postpago.length) return;
            }
            const r = await fetch('prontipagos_proxy.php?servicio=nextel');
            const d = await r.json();
            const list = Array.isArray(d) ? d : [];
            attServices.postpago = list.filter(p => /telefonia/i.test(p.name || ''));
        } catch { attServices.postpago = []; }
    }

    static updateTabVisibility() {
        const body = document.querySelector('.att-main-card');
        if (!body) return;
        ['postpago'].forEach(tab => {
            const btn = body.querySelector(`.tab-btn[data-tab="${tab}"]`);
            const has = Array.isArray(attServices[tab]) && attServices[tab].length > 0;
    ...
    if (!document.getElementById('att-styles')) {
        ...
        #att-kiosk-wrapper { width: 98%; max-width: none; max-height: 96vh; display: flex; align-items: center; justify-content: center; }
        .att-main-card {
          width: 100%; max-height: 96vh; background: white; border-radius: 24px;
          box-shadow: 0 25px 80px rgba(0, 159, 219, 0.4), 0 0 1px rgba(0, 0, 0, 0.2);
          overflow: hidden; animation: slideUp 0.5s cubic-bezier(0.34, 1.56, 0.64, 1);
          border: 1px solid rgba(0, 159, 219, 0.1); display: flex; flex-direction: column; transform-origin: center;
        }
        .att-header {
          background: linear-gradient(135deg, #009FDB 0%, #0076A8 100%); padding: 3rem 3rem;
        ...
        .att-header::after {
          ...
          background: linear-gradient(90deg, #009FDB 0%, #0076A8 50%, #009FDB 100%);
          box-shadow: 0 0 20px rgba(0, 159, 219, 0.5);
        }
        .att-header img {
        ...
        .att-content { padding: 3.5rem; background: #fafbfc; overflow-y: auto; flex: 1; }
        .step-chip {
          display: inline-flex; align-items: center; gap: 0.5rem; background: white; color: #009FDB;
          font-weight: 600; font-size: 1.05rem; padding: 0.85rem 1.75rem; border-radius: 14px;
          border: 2px solid #B3DFF5; margin-bottom: 2.5rem; box-shadow: 0 3px 10px rgba(0, 159, 219, 0.1);
          transition: all 0.3s ease; animation: chipBounce 0.6s ease-out;
        }
        .step-chip:hover { border-color: #009FDB; box-shadow: 0 5px 15px rgba(0, 159, 219, 0.2); transform: translateY(-2px); }
        ...
        .input-wrap input:focus {
          outline: none; border-color: #009FDB; background: white;
          box-shadow: 0 0 0 5px rgba(0, 159, 219, 0.1), 0 8px 16px rgba(0, 159, 219, 0.15); transform: translateY(-2px);
        }
        .state-icon {
        ...
        .hint-card {
          background: white; border: 2px solid #B3DFF5; border-radius: 16px; padding: 1.75rem;
          margin-bottom: 2rem; box-shadow: 0 2px 8px rgba(0, 159, 219, 0.06);
        }
        ...
        .hint-icon {
          ...
          background: linear-gradient(135deg, #009FDB 0%, #0076A8 100%); color: white; border-radius: 50%;
          font-size: 0.8rem; font-weight: 700; box-shadow: 0 2px 8px rgba(0, 159, 219, 0.25);
        }
        .step-header {
        ...
        .btn-primary-telcel {
          background: linear-gradient(135deg, #009FDB 0%, #0076A8 100%); color: white;
          box-shadow: 0 4px 14px rgba(0, 159, 219, 0.35);
        }
        .btn-primary-telcel:hover { transform: translateY(-2px); box-shadow: 0 6px 20px rgba(0, 159, 219, 0.45); }
        .btn-primary-telcel:disabled {
          opacity: 0.5; cursor: not-allowed; transform: none; box-shadow: 0 2px 8px rgba(0, 159, 219, 0.2);
        }
        ...
        .service-item::before {
          ...
          background: linear-gradient(90deg, #009FDB 0%, #0076A8 100%); transform: scaleX(0); transition: transform 0.3s ease;
        }
        .service-item:hover { transform: translateY(-4px); border-color: #009FDB; box-shadow: 0 12px 28px rgba(0, 159, 219, 0.15); }
        .service-item:hover::before { transform: scaleX(1); }
        .service-item.selected {
          border-color: #009FDB; background: linear-gradient(135deg, #e6eeff 0%, #B3DFF5 100%);
          box-shadow: 0 12px 32px rgba(0, 159, 219, 0.25);
        }
        ...
        .service-item .service-price {
          display: inline-block; background: linear-gradient(135deg, #009FDB 0%, #0076A8 100%);
          color: white; padding: 0.6rem 1.2rem; border-radius: 10px; font-weight: 700; font-size: 1.3rem;
          box-shadow: 0 4px 12px rgba(0, 159, 219, 0.25); align-self: flex-start;
        }
        ...
        .tab-btn:hover { border-color: #009FDB; color: #009FDB; }
        .tab-btn.active { background: #009FDB; color: white; border-color: #009FDB; box-shadow: 0 4px 12px rgba(0, 159, 219, 0.3); }
        ...
        #att-kiosk-footer .modern-btn { min-width: 200px; }
        @media (max-width: 768px) {
          #att-kiosk-root { padding: 0; }
          .att-main-card { width: 100%; border-radius: 0; min-height: 100vh; max-width: 100%; max-height: 100vh; }
          .att-header { padding: 2.5rem 2rem; }
          .att-header img { max-width: 150px; }
          .att-header h2 { font-size: 2rem; }
          .att-content { padding: 2.5rem 2rem; }
          ...
          #att-kiosk-footer { padding: 1.25rem 1.5rem; }
          #att-kiosk-footer .modern-btn { width: 100%; padding: 1.25rem 2rem; }
        }
        @media (min-width: 769px) and (max-width: 1200px) {
          .att-main-card { width: 96%; max-width: none; }
          .service-grid { grid-template-columns: repeat(3, 1fr); }
          .att-content { padding: 3rem; }
        }
        @media (min-width: 1201px) {
          .att-main-card { width: 96%; max-width: none; }
          .service-grid { grid-template-columns: repeat(3, 1fr); }
          .att-content { padding: 3.5rem 4.5rem; }
        }
        @media (min-width: 1600px) {
          .att-main-card { width: 94%; max-width: none; }
          .service-grid { grid-template-columns: repeat(4, 1fr); }
          .att-content { padding: 4rem 5rem; }
        }
        `;
        ...
        const bodyHTML = `
          <div class="att-main-card">
            <div class="att-header">
              <img src="images/services/att.png" alt="AT&T" />
              <h2>Servicios AT&T</h2>
              <div class="monedero-badge">
                <span class="monedero-icon">💰</span>
                <span class="monedero-label">Saldo:</span>
                <span class="monedero-amount" id="att-saldo">$${attServices.monederoSaldo.toFixed(2)}</span>
              </div>
            </div>
            ...
            <div class="att-content">
              ...
              <div id="phone-step" class="step-screen">
                ...
                <div class="input-wrap">
                  <input type="tel" id="phone-number" placeholder="Número AT&T (10 dígitos)" inputmode="numeric" pattern="[0-9]{10}" maxlength="10" autofocus />
                ...
                <div class="tab-content">
                  ...
                  <div class="tab-pane" data-tab="postpago" style="display:none;">
                    <div class="postpago-content">
                      ...
                      <div class="input-wrap">
                        <input type="number" id="att-postpago-amount" placeholder="Monto a pagar" min="0.01" step="0.01" />
                      </div>
                      <div id="att-postpago-error" class="error-message">Monto inválido (debe ser mayor a 0)</div>
                      ...
                      <div class="button-group" style="margin-top:2rem;">
                        <button class="modern-btn btn-primary-att" id="att-postpago-pay" disabled>💳 Pagar</button>
                      </div>
                    </div>
                  </div>
                </div>
        ...
        AttServices.openOverlay({ title: 'Servicios AT&T', bodyHTML, footerHTML });
    }
    ...
    static renderList(list) {
        ...
        return list
            ...
            <div class="service-item" data-sku="${s.sku}">
                <div class="service-name">${(s.name || s.formatted_name).replace(/NEXTEL/gi, 'AT&T')}</div>
    ...
    static marcarServiciosInsuficientes() {
        ...
        items.forEach(item => {
            ...
            if (total > attServices.monederoSaldo) {
    ...
    static wire() {
        const body = document.querySelector('.att-main-card');
        const footer = document.getElementById('att-kiosk-footer');
        if (!body || !footer) return;

        if (AttServices._h) {
            const { c1, c2, c3, i1, i2 } = AttServices._h;
        ...
        const validate = () => {
            ...
            // Postpago validation
            const ppAmount = body.querySelector('#att-postpago-amount');
            const ppPay = body.querySelector('#att-postpago-pay');
            const ppError = body.querySelector('#att-postpago-error');
            if (ppAmount && ppPay) {
                const amt = parseFloat(ppAmount.value);
                const validAmt = !isNaN(amt) && amt > 0;
                const hasSku = Array.isArray(attServices.postpago) && attServices.postpago.length > 0;
                if (ppError) ppError.style.display = (ppAmount.value.length > 0 && !validAmt) ? 'block' : 'none';
                ppPay.disabled = !(hasSku && validAmt && match);
            }
            if (match) {
                ...
                const sp = new URLSearchParams({ saldo: '1', svc: 'att', ref }); if (window.ProntiServices?.data) sp.set('data', JSON.stringify(window.ProntiServices.data)); fetch('prontipagos_proxy.php', { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, body: sp }).then(res => res.json()).then(data => {
                    ...
                    if (posicion !== -1) {
                        ...
                        body.querySelector('#att-postpago-amount').value = parseFloat(monto).toFixed(2);
                    }
                });
            }
        };
        ...
        const onClick = (e) => {
            ...
            if (tab) {
                ...
                AttServices.hapticFeedback('light');
                ...
                const ensureLoaded = async () => {
                    if (id === 'tae' && attServices.tae === null) { await AttServices.loadTAE(phone); }
                    if (id === 'postpago' && attServices.postpago === null) { await AttServices.loadPostpago(phone); }
                    if (id !== 'postpago') { AttServices.updateList(id, attServices[id] || []); }
                    setTimeout(() => AttServices.marcarServiciosInsuficientes(), 100);
                };
                if ((id === 'postpago' && attServices.postpago === null) || (paneGrid && !paneGrid.innerHTML.trim())) {
            ...
            if (e.target.closest('#process-service')) {
                ...
                AttServices.hapticFeedback('medium');
                const selected = body.querySelector('.service-item.selected');
                ...
                const name = selected.querySelector('.service-name')?.textContent?.trim() || 'Servicio AT&T';
                ...
                if (window.ServicePaymentHandler) {
                    window.ServicePaymentHandler.showPaymentOptions({
                        ...
                        if (paymentResult && paymentResult.success) {
                            ...
                            await AttServices.mostrarRecargaExitosa(name, phone, price);
                            AttServices.closeOverlay();
                        }
                    });
                } else {
                    ...
                    Swal.fire({
                        ...
                        html: `
                            <div style="text-align: left; padding: 1.5rem; background: #f9fafb; border-radius: 12px;">
                                ...
                                <p style="margin: 0.75rem 0; font-size: 1.1rem;"><strong>📞 Número:</strong> <span style="color: #009FDB;">${phone}</span></p>
                                <hr style="margin: 1rem 0; border: none; border-top: 2px solid #e5e7eb;">
                                <p style="margin: 0.75rem 0; font-size: 1.1rem;"><strong>Monto recarga:</strong> <span style="color: #009FDB;">$${price.toFixed(2)}</span></p>
                                <p style="margin: 0.75rem 0; font-size: 1rem; color: #6b7280;"><strong>Comisión:</strong> +$${comision.toFixed(2)}</p>
                                <hr style="margin: 1rem 0; border: none; border-top: 3px solid #009FDB;">
                        ...
                        if (res.isConfirmed) {
                            ...
                            .then(async (d) => {
                                ...
                                if (d?.codeTransaction === '00') {
                                    ...
                                    await AttServices.mostrarRecargaExitosa(name, phone, price, codeDesc);
                                    AttServices.closeOverlay();
                                } else {
            ...
            // Postpago pay handler
            if (e.target.closest('#att-postpago-pay')) {
                AttServices.hapticFeedback('medium');
                const phone = (body.querySelector('#phone-number')?.value || '').slice(0, 10);
                const prod = Array.isArray(attServices.postpago) ? attServices.postpago[0] : null;
                const amountInput = body.querySelector('#att-postpago-amount');
                const amount = parseFloat(amountInput?.value) || 0;

                if (!isValid10(phone) || !prod || amount <= 0) {
                    Swal.fire('Error', 'Verifica el número y el monto a pagar', 'error');
                    return;
                }

                const name = prod.name || prod.formatted_name || 'Pago Postpago AT&T';
                const sku = prod.sku || '';
                const comision = 2;
                const total = amount + comision;

                if (total > attServices.monederoSaldo) {
                    AttServices.mostrarSaldoInsuficiente(amount);
                    return;
                }
                ...
                if (window.ServicePaymentHandler) {
                    window.ServicePaymentHandler.showPaymentOptions({
                        ...
                        if (paymentResult && paymentResult.success) {
                            await AttServices.mostrarRecargaExitosa(name, phone, amount);
                            AttServices.closeOverlay();
                        }
                    });
                } else {
                    Swal.fire({
                        ...
                        html: `
                            <div style="text-align: left; padding: 1.5rem; background: #f9fafb; border-radius: 12px;">
                                ...
                                <p style="margin: 0.75rem 0; font-size: 1.1rem;"><strong>📞 Número:</strong> <span style="color: #009FDB;">${phone}</span></p>
                                <hr style="margin: 1rem 0; border: none; border-top: 2px solid #e5e7eb;">
                                <p style="margin: 0.75rem 0; font-size: 1.1rem;"><strong>Monto pago:</strong> <span style="color: #009FDB;">$${amount.toFixed(2)}</span></p>
                                <p style="margin: 0.75rem 0; font-size: 1rem; color: #6b7280;"><strong>Comisión:</strong> +$${comision.toFixed(2)}</p>
                                <hr style="margin: 1rem 0; border: none; border-top: 3px solid #009FDB;">
                                <p style="margin: 0.75rem 0; font-size: 1.4rem; text-align: center;"><strong>Total a pagar:</strong> <span style="color: #7c3aed; font-weight: 700;">$${total.toFixed(2)}</span></p>
                            </div>
                        ...
                        if (res.isConfirmed) {
                            ...
                            const params = new URLSearchParams({ comprar: '1', svc: sku, ref: phone, amount: amount });
                            fetch(`prontipagos_proxy.php?${params}`, {
                                headers: { 'Accept': 'application/json' },
                                cache: 'no-store'
                            })
                            ...
                            .then(async (d) => {
                                Swal.close();
                                if (d?.codeTransaction === '00') {
                                    await AttServices.mostrarRecargaExitosa(name, phone, amount, d?.codeDescription);
                                    AttServices.closeOverlay();
                                } else {
                                    Swal.fire({
                                        title: '❌ Error en el pago',
                                        text: d?.codeDescription || 'Ocurrió un error',
                                        icon: 'error',
                                        confirmButtonText: 'Entendido',
                                        confirmButtonColor: '#009FDB'
                                    });
        ...
        if (next) {
            next.addEventListener('click', (ev) => {
                ...
                const match = ok && cok && p === c;
                if (!match) return;
                if (attServices.tae === null) AttServices.loadTAE(p);
                if (attServices.postpago === null) AttServices.loadPostpago(p);
        ...
        body.querySelector('#close-from-phones')?.addEventListener('click', () => { AttServices.closeOverlay(); });
        footer.addEventListener('click', onFooter);
        AttServices._h = { c1: onClick, c2: onInput, c3: onFooter, i1: direct, i2: direct };
        validate();
    }
}

AttServices.openOverlay = ({ title, bodyHTML, footerHTML }) => {
    if (window.ServiceModal) {
        window.ServiceModal.open({ title, bodyHTML, footerHTML });
    } else {
        // Fallback: crear overlay propio
        let root = document.getElementById('att-kiosk-root');
        if (!root) {
            root = document.createElement('div');
            root.id = 'att-kiosk-root';
            document.body.appendChild(root);
            root.innerHTML = `<div id="${id}-kiosk-wrapper">${bodyHTML}</div>`;
            try { AttServices._prev = { h: document.documentElement.style.overflow, b: document.body.style.overflow }; document.documentElement.style.overflow = 'hidden'; document.body.style.overflow = 'hidden'; } catch { }
        }
    }
};

AttServices.closeOverlay = () => {
    AttServices.detenerActualizacionSaldo();
    if (window.ServiceModal) {
        window.ServiceModal.close();
        try { if (AttServices._prev) { document.documentElement.style.overflow = AttServices._prev.h || ''; document.body.style.overflow = AttServices._prev.b || ''; AttServices._prev = null; } } catch { }
    } else {
        const root = document.getElementById('att-kiosk-root');
        if (root) root.remove();
        try { if (AttServices._prev) { document.documentElement.style.overflow = AttServices._prev.h || ''; document.body.style.overflow = AttServices._prev.b || ''; AttServices._prev = null; } } catch { }
    }
};
