    ...
    for ($page = 0; $page <= 6; $page++) {
        ...
        foreach ($pageServices as $service) {
            ...
            if (strpos($name, 'LUZ CFE') !== false) {
                ...
                error_log("Servicio CFE encontrado: " . json_encode($service));
            }
        }
    }
...
function filterCFEServices($services) {
    error_log("Filtrando servicios CFE de " . count($services) . " servicios totales");
    $cfeServices = [];
    ...
    usort($cfeServices, function($a, $b) use ($allowedNames) {
...
try {
    ...
    error_log("Iniciando obtención de servicios CFE...");
