...
function fetchAllServices($token) {
    ...
    for ($page = 0; $page <= 6; $page++) {
        ...
        foreach ($pageServices as $service) {
            ...
            if (strpos($name, 'TELEVISION MEGACABLE') !== false) {
                $allServices[] = [
                    'sku' => $service['sku'] ?? '',
                    'name' => $service['name'] ?? '',
                ...
                error_log("Servicio Megacable encontrado: " . json_encode($service));
            }
        }
    }
...
function filterMegacableServices($services) {
    error_log("Filtrando servicios Megacable de " . count($services) . " servicios totales");
    $megacableServices = [];
    ...
    foreach ($services as $service) {
        ...
        $allowedNames = [
            'TELEVISION MEGACABLE'
        ];
        
        if (in_array($service['name'], $allowedNames)) {
            $megacableServices[] = $service;
        }
    }
    
    usort($megacableServices, function($a, $b) use ($allowedNames) {
        return array_search($a['name'], $allowedNames) - array_search($b['name'], $allowedNames);
    });
    
    return $megacableServices;
}
...
try {
    ...
    error_log("Iniciando obtención de servicios Megacable...");
    ...
    usort($allServices, function($a, $b) {
        $order = [
            'TELEVISION MEGACABLE' => 1,
            'CONSULTA TU SALDO MEGACABLE' => 2
        ];
