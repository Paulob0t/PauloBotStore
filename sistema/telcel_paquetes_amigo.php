...
/**
 * 📱 TELCEL PAQUETES AMIGO - TODOS los SKU sin filtro de región
...
try {
    ...
    error_log("Iniciando obtención de Paquetes Amigo (TODAS las regiones)...");
    ...
    // 🔥 Filtrar SOLO Paquetes Amigo categoría 47 sin región
    $paquetesServices = [];
    ...
    foreach ($allServices as $service) {
        ...
        // Filtrar SOLO categoría 47 (Paquetes Amigo) sin región
        // Buscar SKU que empiece con PA seguido de números, SIN -R
        if ($categoryId === 47) {
            ...
            // Solo si empieza con PA y no hemos agregado ese monto
            if (strpos($sku, 'PA') === 0 && !isset($montosUnicos[$minAmount])) {
                ...
                $paquetesServices[] = [
                    'sku' => $service['sku'] ?? '',
                    'name' => $service['name'] ?? 'Paquete Amigo',
                    ...
                    'region' => $region,
                    'formatted_name' => 'Paquete Amigo $' . number_format($minAmount, 0) . ' MXN',
    ...
    usort($paquetesServices, function($a, $b) {
    ...
    error_log("Total Paquetes Amigo encontrados (SIN región específica): " . count($paquetesServices));
    
    echo json_encode([
        'success' => true,
        'services' => $paquetesServices,
        'total' => count($paquetesServices),
        'message' => 'Paquetes Amigo (generales sin región) cargados exitosamente (' . count($paquetesServices) . ' servicios)',
        'note' => 'Solo se muestran Paquetes generales - se excluyen SKU con región específica (-R1, -R2, etc.)'
    ]);

} catch (Exception $e) {
    error_log("Error en telcel_paquetes_amigo.php: " . $e->getMessage());
    echo json_encode([
        'success' => false,
        'services' => [],
        'error' => $e->getMessage(),
        'message' => 'Error al cargar Paquetes Amigo'
    ]);
}
?>
