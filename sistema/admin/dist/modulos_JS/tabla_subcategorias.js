  ...
  return {
    ...
    initDataTable() {
      this.dataTable = this.elements.tabla.DataTable({
        pageLength: this.config.tabla.pageLength,
        lengthMenu: this.config.tabla.lengthMenu,
        order: [[0, "asc"]],
        language: {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          try {
            ...
            if (data.success) {
              await Swal.fire('Eliminada', 'Subcategoría eliminada correctamente', 'success');
              location.reload();
            } else {
    ...
    async handleSubmitEditar(e) {
      ...
      try {
        ...
        if (result.success) {
          await Swal.fire('Actualizado', 'Subcategoría actualizada correctamente', 'success');
          location.reload();
        } else {
    ...
    async handleSubmitAgregar(e) {
      ...
      try {
        ...
        if (result.success) {
          await Swal.fire('Éxito', 'Subcategoría agregada correctamente', 'success');
          location.reload();
        } else {
