...
try {
    ...
    error_log("Iniciando obtención de Paquetes Amigo Sin Límite Región $regionDetectada...");
    ...
    foreach ($allServices as $service) {
        ...
        $isPaqueteAmigo = false;
        $regionSufijo = '-R' . $regionDetectada;
        
        if ($categoryId === 47) {
            if (strpos($sku, 'PA') === 0 && !str_contains($sku, '-R')) { // Se cambio condicional para tomar solo los paquetes sin region
                // if () {
                    $isPaqueteAmigo = true;
                // }
            }
        ...
        if ($isPaqueteAmigo && $minAmount > 0) {
            $paquetesRegion6[] = [
                ...
                'formatted_name' => 'Paquete Amigo $' . number_format($minAmount, 0) . ' MXN R' . $regionDetectada,
                'price_display' => '$' . number_format($minAmount, 0)
            ];
        }
    }
    
    usort($paquetesRegion6, function($a, $b) {
        return $a['minAmount'] - $b['minAmount'];
    });
    
    error_log("Total Paquetes Amigo Sin Límite Región $regionDetectada encontrados: " . count($paquetesRegion6));
    
    echo json_encode([
        'success' => true,
        'services' => $paquetesRegion6,
        'total' => count($paquetesRegion6),
        'region' => $regionDetectada,
        'phone' => $numeroTelefono,
        'message' => 'Paquetes Amigo Sin Límite Región ' . $regionDetectada . ' cargados exitosamente (' . count($paquetesRegion6) . ' paquetes)'
    ]);
    ...
    echo json_encode([
        ...
        'services' => [],
        'error' => $e->getMessage(),
        'message' => 'Error al cargar Paquetes Amigo Sin Límite'
    ]);
}
?>
