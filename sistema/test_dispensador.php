...
<div class="grid">
  ...
  <!-- SALDO -->
  <div class="card">
    <h2>💵 Saldo en Monedero</h2>
    <div class="saldo-display" id="saldoDisplay">$<?= number_format($saldo, 2) ?></div>
    <div style="text-align:center;font-size:.75rem;color:#555;margin-top:4px">actualiza cada 2s</div>
  </div>
  ...
  <div class="card">
    ...
    <?php endforeach; ?>
    <div style="margin-top:12px;font-size:.75rem;color:#555;text-align:right">
      Total en caja: $<span id="totalInventario"><?= $inventory['total_pesos'] ?? 0 ?></span>
    </div>
  </div>
...
<div style="max-width:1100px;margin:0 auto;padding:0 25px 20px">
  <div class="card">
    ...
    <p style="font-size:.8rem;color:#888;margin-bottom:16px">
      Envía comandos raw al hardware para identificar qué tubo dispensa qué moneda.<br>
      Prueba cada botón y observa qué moneda sale físicamente.
    </p>
    <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin-bottom:16px">
      ...
      $testCmds = [
        'INT0001003'=>'HEX 01 ($1?)',
        'INT0002003'=>'HEX 02 ($2?)',
        'INT0003003'=>'HEX 03 (tubo 3?)',
        'INT0004003'=>'HEX 04 (tubo 4?)',
        'INT0005003'=>'HEX 05 ($5?)',
        'INT0006003'=>'HEX 06 (tubo 6?)',
        'INT000A003'=>'HEX 0A ($10?)',
        'INT0014003'=>'HEX 14 ($20?)',
      ];
...
async function refreshSaldo() {
  try {
    ...
    if (data.saldo !== undefined) {
      document.getElementById('saldoDisplay').textContent = '$' + parseFloat(data.saldo).toFixed(2);
    }
  } catch(e) {}
}
