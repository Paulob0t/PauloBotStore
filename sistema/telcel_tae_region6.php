...
try {
    ...
    error_log("Iniciando obtención de servicios TAE Región $regionDetectada...");
    ...
    foreach ($allServices as $service) {
        ...
        $isTAE = false;
        $regionTexto = 'region ' . $regionDetectada;
        $skuRegion = 'S3TAER' . $regionDetectada;
        
        if (stripos($name, 'tiempo aire ' . $regionTexto) !== false && 
            stripos($name, 'alo') === false && 
            stripos($name, 'recarga') !== false) {
            $isTAE = true;
        }
        
        if (strpos($sku, $skuRegion) !== false && 
            strpos($sku, 'ALO') === false && 
            $minAmount > 0) {
            $isTAE = true;
        }
        ...
        if ($isTAE && $minAmount > 0) {
            $taeRegion6Services[] = [
                'sku' => $service['sku'] ?? '',
                'name' => $service['name'] ?? 'Recarga TAE',
                'description' => $service['description'] ?? '',
                'minAmount' => $minAmount,
                'maxAmount' => floatval($service['maxAmount'] ?? 0),
                'categoryId' => $service['categoryId'] ?? null,
                'discount' => floatval($service['discount'] ?? 0),
                'formatted_name' => 'Recarga TAE $' . number_format($minAmount, 0) . ' MXN',
                'price_display' => '$' . number_format($minAmount, 0)
            ];
        }
    }
    
    usort($taeRegion6Services, function($a, $b) {
        return $a['minAmount'] - $b['minAmount'];
    });
    
    error_log("Total recargas TAE Región $regionDetectada encontradas: " . count($taeRegion6Services));
    
    echo json_encode([
        'success' => true,
        'services' => $taeRegion6Services,
        'total' => count($taeRegion6Services),
        'region' => $regionDetectada,
        'phone' => $numeroTelefono,
        'message' => 'Recargas TAE Región ' . $regionDetectada . ' cargadas exitosamente (' . count($taeRegion6Services) . ' recargas)'
    ]);
    ...
    echo json_encode([
        ...
        'services' => [],
        'error' => $e->getMessage(),
        'message' => 'Error al cargar recargas TAE'
    ]);
}
?>
