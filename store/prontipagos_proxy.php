...
function filterServices($services,$svc,$saldo = 0) {
    ...
    $filtered = array_filter($services, function($service) use ($allowedNames, $saldo) {
        foreach ($allowedNames as $name) {
            if (stripos($service['name'], $name) !== false) {
                if ($saldo == 2)
                    return true;
                if ($saldo == 1 && stripos($service['name'], 'SALDO') !== false)
                    return true;
                if ($saldo == 0 && stripos($service['name'], 'SALDO') === false)
                    return true;
            }
        }
        return false;
...
$servicesData = null;
if (isset($_POST['data'])){
    $servicesData = json_decode($_POST['data'], true);
    $servicesData = ['all' => $servicesData];
} else {        
...
if (isset($_POST['saldo'])){
    ...
    // $tk = authenticateAPI();
    //  $servicesData = fetchAllServices($tk);
    $servicesData['filtered'] = filterServices($servicesData['all'], explode(',', $servicio),1);
    
    if (empty($servicesData['filtered'])) {
        echo json_encode(['additionalInfo' => '$0.00', 'codeTransaction' => '00']);
        exit;
    }
...
if (isset($_GET['servicio'])){
    ...
    // $tk = authenticateAPI();
    $servicesData = fetchAllServices($tk);
    $servicesData['filtered'] = filterServices($servicesData['all'], explode(',', $servicio),2);
