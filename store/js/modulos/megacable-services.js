...
export class MegacableServices {
    ...
    static async loadServices() {
        if (_loadingMegacable) return _loadingMegacable;
        megacableData.services = [];
        _loadingMegacable = (async () => {
            try {
                if (window.ProntiServices) {
                    await window.ProntiServices.load();
                    const raw = window.ProntiServices.getProducts('megacable');
                    const found = raw.find(p => p.sku === 'S3TELEMEGACABMXN');
                    if (found) {
                        megacableData.services = [found];
                        return;
                    }
                }
                const r = await fetch('prontipagos_proxy.php?servicio=megacable');
                const d = await r.json();
                const list = Array.isArray(d) ? d : [];
                const found = list.find(p => p.sku === 'S3TELEMEGACABMXN');
                megacableData.services = found ? [found] : [];
            } catch { megacableData.services = []; }
            finally { _loadingMegacable = null; }
        })();
        return _loadingMegacable;
    }

    static async load() {
        if (megacableData.services === null || megacableData.services.length === 0) await this.loadServices();
    }
        ...
        const bodyHTML = `
          <div class="megacable-main-card">
            <div class="megacable-header">
              <img src="images/services/megacable.png" alt="Megacable" />
              ...
              <div id="mega-step-2" class="step-screen hidden-step">
                ...
                <div class="hint-card">
                  <div class="hint-item">
                    ...
                    <span>Ingresa el monto a pagar por tu servicio Megacable</span>
                  </div>
                </div>
    ...
    static wire() {
        ...
        const validate = () => {
            ...
            if (match) {
                ...
                next.disabled = true;
                const ref = body.querySelector('#megacable-service-number').value;
                const sp = new URLSearchParams({ saldo: '1', svc: 'megacable', ref }); if (window.ProntiServices?.data) sp.set('data', JSON.stringify(window.ProntiServices.data)); fetch('prontipagos_proxy.php', { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, body: sp }).then(res => res.json()).then(data => {
                    let texto = data['additionalInfo'];
                    let posicion = texto.indexOf("$");
                    if (posicion !== -1) {
                        let monto = texto.slice(posicion + 1);
                        body.querySelector('#megacable-amount').value = parseFloat(monto).toFixed(2);
                    }
                    next.disabled = false;
            ...
            const hasSku = Array.isArray(megacableData.services) && megacableData.services.length > 0;
            if (amountInput) { amountInput.disabled = !hasSku; }
        ...
        body.addEventListener('click', (e) => {
            if (e.target.closest('#process-megacable')) {
                ...
                const sku = Array.isArray(megacableData.services) && megacableData.services.length ? megacableData.services[0].sku : '';
                const serviceName = 'Pago Megacable';
                const amount = parseFloat(body.querySelector('#megacable-amount')?.value);
                if (!/^\d{8,12}$/.test(serviceNumber) || !sku || isNaN(amount) || amount <= 0) { Swal.fire('Error', 'Completa todos los campos correctamente', 'error'); return; }
                const commission = 8;
                    ...
                    Swal.fire({ title: '¿Confirmar pago?', html: `<div style="text-align:left; padding:10px;"><p><strong>Servicio:</strong> ${serviceName}</p><p><strong>Número:</strong> ${serviceNumber}</p><p><strong>Monto:</strong> $${amount.toFixed(2)}</p><p><strong>Comisión:</strong> $${commission.toFixed(2)}</p><p><strong>Total:</strong> $${totalAmount.toFixed(2)}</p></div>`, icon: 'question', showCancelButton: true, confirmButtonText: 'Pagar', cancelButtonText: 'Cancelar' }).then((res) => {
                        if (res.isConfirmed) {
                            ...
                            fetch(`prontipagos_proxy.php?${payParams}`, { headers: { 'Accept': 'application/json' }, cache: 'no-store' })
                                .then(r => r.json())
                                .then(d => { if (d?.codeTransaction === '00') { Swal.fire('¡Pago exitoso!', '', 'success').then(() => MegacableServices.closeOverlay()); } else { Swal.fire('Error', d?.codeDescription || 'Error', 'error'); } });
                        }
                    });
                }
            }
...
MegacableServices.openOverlay = ({ title, bodyHTML, footerHTML }) => {
    if (window.ServiceModal) {
        ...
        if (!root) {
            ...
            root.innerHTML = `<div id="megacable-kiosk-wrapper">${bodyHTML}</div>`;
