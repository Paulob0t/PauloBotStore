...
export class UnefonServices {
    static async init() {
        try {
            ...
            Swal.fire('Error', 'No se pudieron cargar los servicios de Unefon', 'error');
        }
    }
    ...
    static async obtenerSaldoMonedero() {
        try {
            ...
            if (data.success) {
                ...
                unefonServices.monederoSaldo = nuevoSaldo;
                this.actualizarBadgeSaldo();
            }
        } catch (error) {
            unefonServices.monederoSaldo = 0;
            this.actualizarBadgeSaldo();
        }
    }

    static actualizarBadgeSaldo() {
        const badgeElement = document.getElementById('unefon-saldo');
        if (badgeElement) {
            const saldoAnterior = parseFloat(badgeElement.textContent.replace(/[^0-9.]/g, '')) || 0;
            const saldoNuevo = unefonServices.monederoSaldo;
            
            // Animación de conteo progresivo solo para el badge de Unefon
        ...
        if (typeof MonederoIntegration !== 'undefined') {
            ...
            if (MonederoIntegration.saldoActual !== unefonServices.monederoSaldo) {
                MonederoIntegration.saldoActual = unefonServices.monederoSaldo;
            }
        }
    }
    ...
    static async descontarSaldo(monto) {
        try {
            ...
            const nuevoSaldo = Math.max(0, unefonServices.monederoSaldo - monto);
    ...
    static iniciarActualizacionSaldo() {
        if (unefonServices.saldoInterval) {
            clearInterval(unefonServices.saldoInterval);
        }
        
        // Sincronizar con MonederoIntegration en lugar de hacer polling independiente
        unefonServices.saldoInterval = setInterval(() => {
            ...
            if (typeof MonederoIntegration !== 'undefined') {
                ...
                if (nuevoSaldo !== unefonServices.monederoSaldo) {
                    unefonServices.monederoSaldo = nuevoSaldo;
                    this.actualizarBadgeSaldo();
    ...
    static detenerActualizacionSaldo() {
        if (unefonServices.saldoInterval) {
            clearInterval(unefonServices.saldoInterval);
            unefonServices.saldoInterval = null;
        }
    }
    ...
    static mostrarSaldoInsuficiente(price) {
        ...
        const faltante = total - unefonServices.monederoSaldo;
        Swal.fire({
            ...
            html: `
                <div style="text-align: center; padding: 1rem;">
                    ...
                    <p style="font-size: 1.1rem; margin-bottom: 0.5rem;">
                        Monto recarga: <strong style="color: #F68B1F;">$${price.toFixed(2)}</strong>
                    </p>
                    ...
                    <p style="font-size: 1rem; color: #6b7280; margin-bottom: 1rem;">
                        Tu saldo actual: <strong style="color: #7c3aed;">$${unefonServices.monederoSaldo.toFixed(2)}</strong>
                    </p>
    ...
    static async mostrarRecargaExitosa(name, phone, price, codeDesc = null) {
        ...
        const saldoInicial = unefonServices.monederoSaldo + totalCobrado; // Saldo antes de descontar
        ...
        await Swal.fire({
            ...
            html: `
                <div style="text-align: left; padding: 1.5rem; background: linear-gradient(135deg, #f0fdf4 0%, #dcfce7 100%); border-radius: 12px; border: 2px solid #86efac;">
                    ...
                    <p style="margin: 0.75rem 0; font-size: 1.05rem;"><strong>📞 Número:</strong> <span style="color: #F68B1F;">${phone}</span></p>
    ...
    static hapticFeedback(type = 'light') {
        if (!unefonServices.hapticEnabled) return;
    ...
    static showConfetti() {
        const colors = ['#F68B1F', '#0066cc', '#00d4ff', '#10b981', '#f59e0b'];
    ...
    static async loadTAE(phone = '') {
        unefonServices.loadingServices = true;
        ...
        try {
            if (window.ProntiServices) {
                ...
                const raw = window.ProntiServices.getProducts('unefon');
                const recargas = (raw || []).filter(s => !/paquete|internet|saldo/i.test(s.name || ''));
                if (recargas.length) {
                    unefonServices.tae = recargas;
                    return;
                }
            }
            unefonServices.tae = TAE_AMOUNTS.map(item => ({
                sku: '',
                formatted_name: item.label,
                description: 'Recarga Unefon',
    ...
    static render() {
    if (!document.getElementById('unefon-styles')) {
        const style = document.createElement('style');
        style.id = 'unefon-styles';
        style.textContent = `
        ...
        #unefon-kiosk-wrapper { width: 98%; max-width: none; max-height: 96vh; display: flex; align-items: center; justify-content: center; }
        .unefon-main-card {
          width: 100%; max-height: 96vh; background: white; border-radius: 24px;
          box-shadow: 0 25px 80px rgba(246, 139, 31, 0.4), 0 0 1px rgba(0, 0, 0, 0.2);
          overflow: hidden; animation: slideUp 0.5s cubic-bezier(0.34, 1.56, 0.64, 1);
          border: 1px solid rgba(246, 139, 31, 0.1); display: flex; flex-direction: column; transform-origin: center;
        }
        .unefon-header {
          background: linear-gradient(135deg, #F68B1F 0%, #D97700 100%); padding: 3rem 3rem;
          text-align: center; position: relative; overflow: hidden; flex-shrink: 0;
        }
        .unefon-header::before {
        ...
        .unefon-header::after {
          ...
          background: linear-gradient(90deg, #F68B1F 0%, #D97700 50%, #F68B1F 100%);
        ...
        .unefon-content { padding: 3.5rem; background: #fafbfc; overflow-y: auto; flex: 1; }
        .step-chip {
          display: inline-flex; align-items: center; gap: 0.5rem; background: white; color: #F68B1F;
          font-weight: 600; font-size: 1.05rem; padding: 0.85rem 1.75rem; border-radius: 14px;
          border: 2px solid #FCD5A8; margin-bottom: 2.5rem; box-shadow: 0 3px 10px rgba(246, 139, 31, 0.1);
          transition: all 0.3s ease; animation: chipBounce 0.6s ease-out;
        }
        .step-chip:hover { border-color: #F68B1F; box-shadow: 0 5px 15px rgba(246, 139, 31, 0.2); transform: translateY(-2px); }
        ...
        .input-wrap input:focus {
          outline: none; border-color: #F68B1F; background: white;
          box-shadow: 0 0 0 5px rgba(246, 139, 31, 0.1), 0 8px 16px rgba(246, 139, 31, 0.15); transform: translateY(-2px);
        }
        .state-icon {
        ...
        .hint-card {
          background: white; border: 2px solid #FCD5A8; border-radius: 16px; padding: 1.75rem;
          margin-bottom: 2rem; box-shadow: 0 2px 8px rgba(246, 139, 31, 0.06);
        }
        ...
        .hint-icon {
          ...
          background: linear-gradient(135deg, #F68B1F 0%, #D97700 100%); color: white; border-radius: 50%;
          font-size: 0.8rem; font-weight: 700; box-shadow: 0 2px 8px rgba(246, 139, 31, 0.25);
        }
        .step-header {
        ...
        .btn-primary-telcel {
          background: linear-gradient(135deg, #F68B1F 0%, #D97700 100%); color: white;
          box-shadow: 0 4px 14px rgba(246, 139, 31, 0.35);
        }
        .btn-primary-telcel:hover { transform: translateY(-2px); box-shadow: 0 6px 20px rgba(246, 139, 31, 0.45); }
        .btn-primary-telcel:disabled {
          opacity: 0.5; cursor: not-allowed; transform: none; box-shadow: 0 2px 8px rgba(246, 139, 31, 0.2);
        }
        ...
        .service-item::before {
          ...
          background: linear-gradient(90deg, #F68B1F 0%, #D97700 100%); transform: scaleX(0); transition: transform 0.3s ease;
        }
        .service-item:hover { transform: translateY(-4px); border-color: #F68B1F; box-shadow: 0 12px 28px rgba(246, 139, 31, 0.15); }
        .service-item:hover::before { transform: scaleX(1); }
        .service-item.selected {
          border-color: #F68B1F; background: linear-gradient(135deg, #e6eeff 0%, #FCD5A8 100%);
        ...
        .service-item .service-price {
          display: inline-block; background: linear-gradient(135deg, #F68B1F 0%, #D97700 100%);
          color: white; padding: 0.6rem 1.2rem; border-radius: 10px; font-weight: 700; font-size: 1.3rem;
          box-shadow: 0 4px 12px rgba(246, 139, 31, 0.25); align-self: flex-start;
        }
        ...
        .tab-btn:hover { border-color: #F68B1F; color: #F68B1F; }
        .tab-btn.active { background: #F68B1F; color: white; border-color: #F68B1F; box-shadow: 0 4px 12px rgba(246, 139, 31, 0.3); }
        .tab-content { margin-top: 1rem; }
        @media (max-width: 768px) {
          #unefon-kiosk-root { padding: 0; }
          .unefon-main-card { width: 100%; border-radius: 0; min-height: 100vh; max-width: 100%; max-height: 100vh; }
          .unefon-header { padding: 2.5rem 2rem; }
          .unefon-header img { max-width: 150px; }
          .unefon-header h2 { font-size: 2rem; }
          .unefon-content { padding: 2.5rem 2rem; }
        ...
        @media (min-width: 769px) and (max-width: 1200px) {
          .unefon-main-card { width: 96%; max-width: none; }
          .service-grid { grid-template-columns: repeat(3, 1fr); }
          .unefon-content { padding: 3rem; }
        }
        @media (min-width: 1201px) {
          .unefon-main-card { width: 96%; max-width: none; }
          .service-grid { grid-template-columns: repeat(3, 1fr); }
          .unefon-content { padding: 3.5rem 4.5rem; }
        }
        @media (min-width: 1600px) {
          .unefon-main-card { width: 94%; max-width: none; }
          .service-grid { grid-template-columns: repeat(4, 1fr); }
          .unefon-content { padding: 4rem 5rem; }
        }
        `;
        ...
        const bodyHTML = `
          <div class="unefon-main-card">
            <div class="unefon-header">
              <img src="images/services/unefon.png" alt="Unefon" />
              <h2>Servicios Unefon</h2>
              <div class="monedero-badge">
                <span class="monedero-icon">💰</span>
                <span class="monedero-label">Saldo:</span>
                <span class="monedero-amount" id="unefon-saldo">$${unefonServices.monederoSaldo.toFixed(2)}</span>
              </div>
            </div>
            
            <div class="unefon-content">
              ...
              <div id="phone-step" class="step-screen">
                ...
                <div class="input-wrap">
                  <input type="tel" id="phone-number" placeholder="Número Unefon (10 dígitos)" inputmode="numeric" pattern="[0-9]{10}" maxlength="10" autofocus />
              ...
              <div id="tabs-step" class="service-tabs step-screen hidden-step">
                ...
                <div class="button-group" style="margin-top: 3rem;">
                  <button type="button" class="modern-btn btn-secondary-telcel" id="close-from-tabs">Cerrar</button>
                  <button type="button" class="modern-btn btn-primary-telcel" id="process-service" style="display:none;" disabled>Pagar</button>
                </div>
              </div>
            </div>
        ...
        UnefonServices.openOverlay({ title: 'Servicios Unefon', bodyHTML, footerHTML });
    }
    ...
    static marcarServiciosInsuficientes() {
        ...
        items.forEach(item => {
            ...
            if (total > unefonServices.monederoSaldo) {
    ...
    static wire() {
        const body = document.querySelector('.unefon-main-card');
        if (!body) return;

        if (UnefonServices._h) {
            const { c1, c2, i1, i2 } = UnefonServices._h;
        ...
        const onClick = (e) => {
            ...
            if (tab) {
                ...
                UnefonServices.hapticFeedback('light');
                ...
                const ensureLoaded = async () => {
                    if (id === 'tae' && unefonServices.tae === null) { await UnefonServices.loadTAE(phone); }
                    UnefonServices.updateList(id, unefonServices[id] || []);
                    // Forzar actualización de marcas después de cargar
                    setTimeout(() => UnefonServices.marcarServiciosInsuficientes(), 100);
                };
            ...
            if (e.target.closest('#process-service')) {
                ...
                UnefonServices.hapticFeedback('medium');
                const selected = body.querySelector('.service-item.selected');
                ...
                const name = selected.querySelector('.service-name')?.textContent?.trim() || 'Servicio Unefon';
                ...
                if (window.ServicePaymentHandler) {
                    window.ServicePaymentHandler.showPaymentOptions({
                        ...
                        if (paymentResult && paymentResult.success) {
                            ...
                            await UnefonServices.mostrarRecargaExitosa(name, phone, price);
                            UnefonServices.closeOverlay();
                        }
                    });
                } else {
                    ...
                    Swal.fire({
                        ...
                        html: `
                            <div style="text-align: left; padding: 1.5rem; background: #f9fafb; border-radius: 12px;">
                                ...
                                <p style="margin: 0.75rem 0; font-size: 1.1rem;"><strong>📞 Número:</strong> <span style="color: #F68B1F;">${phone}</span></p>
                                <hr style="margin: 1rem 0; border: none; border-top: 2px solid #e5e7eb;">
                                <p style="margin: 0.75rem 0; font-size: 1.1rem;"><strong>Monto recarga:</strong> <span style="color: #F68B1F;">$${price.toFixed(2)}</span></p>
                                <p style="margin: 0.75rem 0; font-size: 1rem; color: #6b7280;"><strong>Comisión:</strong> +$${comision.toFixed(2)}</p>
                                <hr style="margin: 1rem 0; border: none; border-top: 3px solid #F68B1F;">
                        ...
                        if (res.isConfirmed) {
                            ...
                            .then(async (d) => {
                                ...
                                if (d?.codeTransaction === '00') {
                                    ...
                                    await UnefonServices.mostrarRecargaExitosa(name, phone, price, codeDesc);
                                    UnefonServices.closeOverlay();
                                } else {
        ...
        if (next) {
            next.addEventListener('click', (ev) => {
                ...
                const match = ok && cok && p === c;
                if (!match) return;
                if (unefonServices.tae === null) UnefonServices.loadTAE(p);
        ...
        body.querySelector('#close-from-phones')?.addEventListener('click', () => { UnefonServices.closeOverlay(); });
        body.querySelector('#close-from-tabs')?.addEventListener('click', () => { UnefonServices.closeOverlay(); });
        UnefonServices._h = { c1: onClick, c2: onInput, i1: direct, i2: direct };
        validate();
    }
}

UnefonServices.openOverlay = ({ title, bodyHTML, footerHTML }) => {
    if (window.ServiceModal) {
        window.ServiceModal.open({ title, bodyHTML, footerHTML });
    } else {
        // Fallback: crear overlay propio
        let root = document.getElementById('unefon-kiosk-root');
        if (!root) {
            root = document.createElement('div');
            root.id = 'unefon-kiosk-root';
            document.body.appendChild(root);
            root.innerHTML = `<div id="${id}-kiosk-wrapper">${bodyHTML}</div>`;
            try { UnefonServices._prev = { h: document.documentElement.style.overflow, b: document.body.style.overflow }; document.documentElement.style.overflow = 'hidden'; document.body.style.overflow = 'hidden'; } catch { }
        }
    }
};

UnefonServices.closeOverlay = () => {
    UnefonServices.detenerActualizacionSaldo();
    if (window.ServiceModal) {
        window.ServiceModal.close();
        try { if (UnefonServices._prev) { document.documentElement.style.overflow = UnefonServices._prev.h || ''; document.body.style.overflow = UnefonServices._prev.b || ''; UnefonServices._prev = null; } } catch { }
    } else {
        const root = document.getElementById('unefon-kiosk-root');
        if (root) root.remove();
        try { if (UnefonServices._prev) { document.documentElement.style.overflow = UnefonServices._prev.h || ''; document.body.style.overflow = UnefonServices._prev.b || ''; UnefonServices._prev = null; } } catch { }
    }
};
