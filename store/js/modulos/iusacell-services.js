...
export class IusacellServices {
    static async init() {
        try {
            ...
            Swal.fire('Error', 'No se pudieron cargar los servicios de Iusacell', 'error');
        }
    }
    ...
    static async obtenerSaldoMonedero() {
        try {
            ...
            if (data.success) {
                ...
                iusacellServices.monederoSaldo = nuevoSaldo;
                this.actualizarBadgeSaldo();
            }
        } catch (error) {
            iusacellServices.monederoSaldo = 0;
            this.actualizarBadgeSaldo();
        }
    }

    static actualizarBadgeSaldo() {
        const badgeElement = document.getElementById('iusacell-saldo');
        if (badgeElement) {
            const saldoAnterior = parseFloat(badgeElement.textContent.replace(/[^0-9.]/g, '')) || 0;
            const saldoNuevo = iusacellServices.monederoSaldo;
            
            // Animación de conteo progresivo solo para el badge de Iusacell
        ...
        if (typeof MonederoIntegration !== 'undefined') {
            ...
            if (MonederoIntegration.saldoActual !== iusacellServices.monederoSaldo) {
                MonederoIntegration.saldoActual = iusacellServices.monederoSaldo;
            }
        }
    }
    ...
    static async descontarSaldo(monto) {
        try {
            ...
            const nuevoSaldo = Math.max(0, iusacellServices.monederoSaldo - monto);
    ...
    static iniciarActualizacionSaldo() {
        if (iusacellServices.saldoInterval) {
            clearInterval(iusacellServices.saldoInterval);
        }
        
        // Sincronizar con MonederoIntegration en lugar de hacer polling independiente
        iusacellServices.saldoInterval = setInterval(() => {
            ...
            if (typeof MonederoIntegration !== 'undefined') {
                ...
                if (nuevoSaldo !== iusacellServices.monederoSaldo) {
                    iusacellServices.monederoSaldo = nuevoSaldo;
                    this.actualizarBadgeSaldo();
    ...
    static detenerActualizacionSaldo() {
        if (iusacellServices.saldoInterval) {
            clearInterval(iusacellServices.saldoInterval);
            iusacellServices.saldoInterval = null;
        }
    }
    ...
    static mostrarSaldoInsuficiente(price) {
        ...
        const faltante = total - iusacellServices.monederoSaldo;
        Swal.fire({
            ...
            html: `
                <div style="text-align: center; padding: 1rem;">
                    ...
                    <p style="font-size: 1.1rem; margin-bottom: 0.5rem;">
                        Monto recarga: <strong style="color: #8E2D89;">$${price.toFixed(2)}</strong>
                    </p>
                    ...
                    <p style="font-size: 1rem; color: #6b7280; margin-bottom: 1rem;">
                        Tu saldo actual: <strong style="color: #7c3aed;">$${iusacellServices.monederoSaldo.toFixed(2)}</strong>
                    </p>
    ...
    static async mostrarRecargaExitosa(name, phone, price, codeDesc = null) {
        ...
        const saldoInicial = iusacellServices.monederoSaldo + totalCobrado; // Saldo antes de descontar
        ...
        await Swal.fire({
            ...
            html: `
                <div style="text-align: left; padding: 1.5rem; background: linear-gradient(135deg, #f0fdf4 0%, #dcfce7 100%); border-radius: 12px; border: 2px solid #86efac;">
                    ...
                    <p style="margin: 0.75rem 0; font-size: 1.05rem;"><strong>📞 Número:</strong> <span style="color: #8E2D89;">${phone}</span></p>
    ...
    static hapticFeedback(type = 'light') {
        if (!iusacellServices.hapticEnabled) return;
    ...
    static showConfetti() {
        const colors = ['#8E2D89', '#0066cc', '#00d4ff', '#10b981', '#f59e0b'];
    ...
    static async loadTAE(phone = '') {
        iusacellServices.loadingServices = true;
        ...
        try {
            if (window.ProntiServices) {
                ...
                const raw = window.ProntiServices.getProducts('iusacell');
                const recargas = (raw || []).filter(s => !/paquete|internet|saldo|telefonia/i.test(s.name || ''));
                if (recargas.length) {
                    iusacellServices.tae = recargas;
                    return;
                }
            }
            iusacellServices.tae = TAE_AMOUNTS.map(item => ({
                sku: '',
                formatted_name: item.label,
                description: 'Recarga Iusacell',
                price_display: `$${item.amount.toFixed(2)}`,
                amount: item.amount
            }));
        } catch (err) { 
            iusacellServices.tae = []; 
        }
    }

    static async loadPostpago(phone = '') {
        try {
            if (window.ProntiServices) {
                await window.ProntiServices.load();
                const raw = window.ProntiServices.getProducts('iusacell');
                iusacellServices.postpago = raw.filter(p => /telefonia/i.test(p.name || ''));
                if (iusacellServices.postpago.length) return;
            }
            const r = await fetch('prontipagos_proxy.php?servicio=iusacell');
            const d = await r.json();
            const list = Array.isArray(d) ? d : [];
            iusacellServices.postpago = list.filter(p => /telefonia/i.test(p.name || ''));
        } catch { iusacellServices.postpago = []; }
    }

    static updateTabVisibility() {
        const body = document.querySelector('.iusacell-main-card');
        if (!body) return;
        ['postpago'].forEach(tab => {
            const btn = body.querySelector(`.tab-btn[data-tab="${tab}"]`);
            const has = Array.isArray(iusacellServices[tab]) && iusacellServices[tab].length > 0;
    ...
    static render() {
    if (!document.getElementById('iusacell-styles')) {
        const style = document.createElement('style');
        style.id = 'iusacell-styles';
        style.textContent = `
        ...
        #iusacell-kiosk-wrapper { width: 98%; max-width: none; max-height: 96vh; display: flex; align-items: center; justify-content: center; }
        .iusacell-main-card {
          width: 100%; max-height: 96vh; background: white; border-radius: 24px;
          box-shadow: 0 25px 80px rgba(142, 45, 137, 0.4), 0 0 1px rgba(0, 0, 0, 0.2);
          overflow: hidden; animation: slideUp 0.5s cubic-bezier(0.34, 1.56, 0.64, 1);
          border: 1px solid rgba(142, 45, 137, 0.1); display: flex; flex-direction: column; transform-origin: center;
        }
        .iusacell-header {
          background: linear-gradient(135deg, #8E2D89 0%, #6B1F67 100%); padding: 3rem 3rem;
          text-align: center; position: relative; overflow: hidden; flex-shrink: 0;
        }
        .iusacell-header::before {
          content: ""; position: absolute; top: 0; left: 0; right: 0; bottom: 0;
          background: radial-gradient(circle at 20% 30%, rgba(255,255,255,0.15) 0%, transparent 50%),
                      radial-gradient(circle at 80% 70%, rgba(255,255,255,0.1) 0%, transparent 50%);
          animation: headerPulse 4s ease-in-out infinite;
        }
        .iusacell-header::after {
          content: ""; position: absolute; top: 0; left: 0; right: 0; height: 5px;
          background: linear-gradient(90deg, #8E2D89 0%, #6B1F67 50%, #8E2D89 100%);
          box-shadow: 0 0 20px rgba(142, 45, 137, 0.5);
        }
        .iusacell-header img {
        ...
        .iusacell-content { padding: 3.5rem; background: #fafbfc; overflow-y: auto; flex: 1; }
        .step-chip {
          display: inline-flex; align-items: center; gap: 0.5rem; background: white; color: #8E2D89;
          font-weight: 600; font-size: 1.05rem; padding: 0.85rem 1.75rem; border-radius: 14px;
          border: 2px solid #D4A3D1; margin-bottom: 2.5rem; box-shadow: 0 3px 10px rgba(142, 45, 137, 0.1);
          transition: all 0.3s ease; animation: chipBounce 0.6s ease-out;
        }
        .step-chip:hover { border-color: #8E2D89; box-shadow: 0 5px 15px rgba(142, 45, 137, 0.2); transform: translateY(-2px); }
        ...
        .input-wrap input:focus {
          outline: none; border-color: #8E2D89; background: white;
          box-shadow: 0 0 0 5px rgba(142, 45, 137, 0.1), 0 8px 16px rgba(142, 45, 137, 0.15); transform: translateY(-2px);
        }
        .state-icon {
        ...
        .hint-card {
          background: white; border: 2px solid #D4A3D1; border-radius: 16px; padding: 1.75rem;
          margin-bottom: 2rem; box-shadow: 0 2px 8px rgba(142, 45, 137, 0.06);
        }
        ...
        .hint-icon {
          ...
          background: linear-gradient(135deg, #8E2D89 0%, #6B1F67 100%); color: white; border-radius: 50%;
          font-size: 0.8rem; font-weight: 700; box-shadow: 0 2px 8px rgba(142, 45, 137, 0.25);
        }
        .step-header {
        ...
        .btn-primary-telcel {
          background: linear-gradient(135deg, #8E2D89 0%, #6B1F67 100%); color: white;
          box-shadow: 0 4px 14px rgba(142, 45, 137, 0.35);
        }
        .btn-primary-telcel:hover { transform: translateY(-2px); box-shadow: 0 6px 20px rgba(142, 45, 137, 0.45); }
        .btn-primary-telcel:disabled {
          opacity: 0.5; cursor: not-allowed; transform: none; box-shadow: 0 2px 8px rgba(142, 45, 137, 0.2);
        }
        ...
        .service-item::before {
          ...
          background: linear-gradient(90deg, #8E2D89 0%, #6B1F67 100%); transform: scaleX(0); transition: transform 0.3s ease;
        }
        .service-item:hover { transform: translateY(-4px); border-color: #8E2D89; box-shadow: 0 12px 28px rgba(142, 45, 137, 0.15); }
        .service-item:hover::before { transform: scaleX(1); }
        .service-item.selected {
          border-color: #8E2D89; background: linear-gradient(135deg, #e6eeff 0%, #D4A3D1 100%);
          box-shadow: 0 12px 32px rgba(142, 45, 137, 0.25);
        }
        ...
        .service-item .service-price {
          display: inline-block; background: linear-gradient(135deg, #8E2D89 0%, #6B1F67 100%);
          color: white; padding: 0.6rem 1.2rem; border-radius: 10px; font-weight: 700; font-size: 1.3rem;
          box-shadow: 0 4px 12px rgba(142, 45, 137, 0.25); align-self: flex-start;
        }
        ...
        .tab-btn:hover { border-color: #8E2D89; color: #8E2D89; }
        .tab-btn.active { background: #8E2D89; color: white; border-color: #8E2D89; box-shadow: 0 4px 12px rgba(142, 45, 137, 0.3); }
        .tab-content { margin-top: 1rem; }
        #iusacell-kiosk-footer {
        ...
        #iusacell-kiosk-footer .modern-btn { min-width: 200px; }
        @media (max-width: 768px) {
          #iusacell-kiosk-root { padding: 0; }
          .iusacell-main-card { width: 100%; border-radius: 0; min-height: 100vh; max-width: 100%; max-height: 100vh; }
          .iusacell-header { padding: 2.5rem 2rem; }
          .iusacell-header img { max-width: 150px; }
          .iusacell-header h2 { font-size: 2rem; }
          .iusacell-content { padding: 2.5rem 2rem; }
          ...
          #iusacell-kiosk-footer { padding: 1.25rem 1.5rem; }
          #iusacell-kiosk-footer .modern-btn { width: 100%; padding: 1.25rem 2rem; }
        }
        @media (min-width: 769px) and (max-width: 1200px) {
          .iusacell-main-card { width: 96%; max-width: none; }
          .service-grid { grid-template-columns: repeat(3, 1fr); }
          .iusacell-content { padding: 3rem; }
        }
        @media (min-width: 1201px) {
          .iusacell-main-card { width: 96%; max-width: none; }
          .service-grid { grid-template-columns: repeat(3, 1fr); }
          .iusacell-content { padding: 3.5rem 4.5rem; }
        }
        @media (min-width: 1600px) {
          .iusacell-main-card { width: 94%; max-width: none; }
          .service-grid { grid-template-columns: repeat(4, 1fr); }
          .iusacell-content { padding: 4rem 5rem; }
        }
        `;
        ...
        const bodyHTML = `
          <div class="iusacell-main-card">
            <div class="iusacell-header">
              <img src="images/services/iusacell.png" alt="Iusacell" />
              <h2>Servicios Iusacell</h2>
              <div class="monedero-badge">
                <span class="monedero-icon">💰</span>
                <span class="monedero-label">Saldo:</span>
                <span class="monedero-amount" id="iusacell-saldo">$${iusacellServices.monederoSaldo.toFixed(2)}</span>
              </div>
            </div>
            
            <div class="iusacell-content">
              ...
              <div id="phone-step" class="step-screen">
                ...
                <div class="input-wrap">
                  <input type="tel" id="phone-number" placeholder="Número Iusacell (10 dígitos)" inputmode="numeric" pattern="[0-9]{10}" maxlength="10" autofocus />
                ...
                <div class="tab-content">
                  ...
                  <div class="tab-pane" data-tab="postpago" style="display:none;">
                    <div class="postpago-content">
                      ...
                      <div class="input-wrap">
                        <input type="number" id="iusacell-postpago-amount" placeholder="Monto a pagar" min="0.01" step="0.01" />
                      </div>
                      <div id="iusacell-postpago-error" class="error-message">Monto inválido (debe ser mayor a 0)</div>
                      ...
                      <div class="button-group" style="margin-top:2rem;">
                        <button class="modern-btn btn-primary-iusacell" id="iusacell-postpago-pay" disabled>💳 Pagar</button>
                      </div>
                    </div>
                  </div>
                </div>
        ...
        IusacellServices.openOverlay({ title: 'Servicios Iusacell', bodyHTML, footerHTML });
    }
    ...
    static marcarServiciosInsuficientes() {
        ...
        items.forEach(item => {
            ...
            if (total > iusacellServices.monederoSaldo) {
    ...
    static wire() {
        const body = document.querySelector('.iusacell-main-card');
        const footer = document.getElementById('iusacell-kiosk-footer');
        if (!body || !footer) return;

        if (IusacellServices._h) {
            const { c1, c2, c3, i1, i2 } = IusacellServices._h;
        ...
        const validate = () => {
            ...
            const postpagoAmount = body.querySelector('#iusacell-postpago-amount');
            const postpagoError = body.querySelector('#iusacell-postpago-error');
            const postpagoPayBtn = body.querySelector('#iusacell-postpago-pay');
            if (postpagoAmount && postpagoPayBtn) {
                const amt = parseFloat(postpagoAmount.value) || 0;
                const hasSku = Array.isArray(iusacellServices.postpago) && iusacellServices.postpago.length > 0;
                const valid = amt > 0 && hasSku && match;
                if (postpagoError) {
                    postpagoError.style.display = (postpagoAmount.value.length > 0 && !(amt > 0)) ? 'block' : 'none';
                }
                postpagoPayBtn.disabled = !valid;
            }
            if (match) {
                const ref = body.querySelector('#phone-number').value;
                const sp = new URLSearchParams({ saldo: '1', svc: 'att', ref }); if (window.ProntiServices?.data) sp.set('data', JSON.stringify(window.ProntiServices.data)); fetch('prontipagos_proxy.php', { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, body: sp }).then(res => res.json()).then(data => {
                    let texto = data['additionalInfo'];
                    let posicion = texto.indexOf("$");
                    if (posicion !== -1) {
                        let monto = texto.slice(posicion + 1);
                        body.querySelector('#iusacell-postpago-amount').value = parseFloat(monto).toFixed(2);
                    }
                });
            }
        };
        ...
        const onClick = (e) => {
            ...
            if (tab) {
                ...
                IusacellServices.hapticFeedback('light');
                ...
                const ensureLoaded = async () => {
                    if (id === 'tae' && iusacellServices.tae === null) { await IusacellServices.loadTAE(phone); }
                    if (id === 'postpago' && iusacellServices.postpago === null) { await IusacellServices.loadPostpago(phone); }
                    if (id !== 'postpago') { IusacellServices.updateList(id, iusacellServices[id] || []); }
                    setTimeout(() => IusacellServices.marcarServiciosInsuficientes(), 100);
                };
                if ((id === 'postpago' && iusacellServices.postpago === null) || (paneGrid && !paneGrid.innerHTML.trim())) {
            ...
            if (item) {
                if (item.classList.contains('insufficient-funds')) {
                    // Vibración de error
                    IusacellServices.hapticFeedback('error');
                    const price = parseFloat(item.querySelector('.service-price')?.textContent.replace(/[^0-9.]/g, '')) || 0;
                    IusacellServices.mostrarSaldoInsuficiente(price);
                    return;
                }
            ...
            if (e.target.closest('#process-service')) {
                ...
                IusacellServices.hapticFeedback('medium');
                const selected = body.querySelector('.service-item.selected');
                ...
                const name = selected.querySelector('.service-name')?.textContent?.trim() || 'Servicio Iusacell';
                ...
                if (window.ServicePaymentHandler) {
                    window.ServicePaymentHandler.showPaymentOptions({
                        ...
                        if (paymentResult && paymentResult.success) {
                            ...
                            await IusacellServices.mostrarRecargaExitosa(name, phone, price);
                            IusacellServices.closeOverlay();
                        }
                    });
                } else {
                    ...
                    Swal.fire({
                        ...
                        html: `
                            <div style="text-align: left; padding: 1.5rem; background: #f9fafb; border-radius: 12px;">
                                ...
                                <p style="margin: 0.75rem 0; font-size: 1.1rem;"><strong>📞 Número:</strong> <span style="color: #8E2D89;">${phone}</span></p>
                                <hr style="margin: 1rem 0; border: none; border-top: 2px solid #e5e7eb;">
                                <p style="margin: 0.75rem 0; font-size: 1.1rem;"><strong>Monto recarga:</strong> <span style="color: #8E2D89;">$${price.toFixed(2)}</span></p>
                                <p style="margin: 0.75rem 0; font-size: 1rem; color: #6b7280;"><strong>Comisión:</strong> +$${comision.toFixed(2)}</p>
                                <hr style="margin: 1rem 0; border: none; border-top: 3px solid #8E2D89;">
                        ...
                        if (res.isConfirmed) {
                                ...
                                html: `
                                    <div style="padding: 1rem;">
                                        ...
                                        <div style="margin-top: 1.5rem; text-align: center;">
                                            <div class="iusacell-spinner"></div>
                                        </div>
                                    </div>
                                `,
                            ...
                            .then(async (d) => {
                                ...
                                if (d?.codeTransaction === '00') {
                                    ...
                                    await IusacellServices.mostrarRecargaExitosa(name, phone, price, codeDesc);
                                    IusacellServices.closeOverlay();
                                } else {
            ...
            // Manejar click en botón pagar postpago
            if (e.target.closest('#iusacell-postpago-pay')) {
                IusacellServices.hapticFeedback('medium');
                const phone = (body.querySelector('#phone-number')?.value || '').slice(0, 10);
                const amountVal = parseFloat(body.querySelector('#iusacell-postpago-amount')?.value) || 0;
                const product = Array.isArray(iusacellServices.postpago) ? iusacellServices.postpago[0] : null;
                
                if (!product || !isValid10(phone) || amountVal <= 0) {
                    Swal.fire('Error', 'Verifica el número y monto ingresado', 'error');
                    return;
                }
                
                const name = product.name || product.formatted_name || 'Pago Postpago Iusacell';
                const sku = product.sku || '';
                const price = amountVal;
                const comision = 2;
                const total = price + comision;
                    ...
                    Swal.fire({
                        ...
                        html: `
                            <div style="text-align: left; padding: 1.5rem; background: #f9fafb; border-radius: 12px;">
                                ...
                                <p style="margin: 0.75rem 0; font-size: 1.1rem;"><strong>📞 Número:</strong> <span style="color: #8E2D89;">${phone}</span></p>
                                <hr style="margin: 1rem 0; border: none; border-top: 2px solid #e5e7eb;">
                                <p style="margin: 0.75rem 0; font-size: 1.1rem;"><strong>Monto pago:</strong> <span style="color: #8E2D89;">$${price.toFixed(2)}</span></p>
                                <p style="margin: 0.75rem 0; font-size: 1rem; color: #6b7280;"><strong>Comisión:</strong> +$${comision.toFixed(2)}</p>
                                <hr style="margin: 1rem 0; border: none; border-top: 3px solid #8E2D89;">
                                <p style="margin: 0.75rem 0; font-size: 1.4rem; text-align: center;"><strong>Total a pagar:</strong> <span style="color: #7c3aed; font-weight: 700;">$${total.toFixed(2)}</span></p>
                            </div>
                        ...
                        if (res.isConfirmed) {
                            Swal.fire({
                                ...
                                html: '<div style="padding:1rem;"><p style="color:#64748b;">Por favor espera un momento</p></div>',
                                allowOutsideClick: false,
                                showConfirmButton: false,
                                didOpen: () => Swal.showLoading()
                            });
                            ...
                            .then(async (d) => {
                                ...
                                if (d?.codeTransaction === '00') {
                                    ...
                                    let errorMessage = d?.codeDescription || 'Ocurrió un error al procesar el pago';
                                    Swal.fire({ title: '❌ Error en el pago', text: errorMessage, icon: 'error', confirmButtonText: 'Entendido', confirmButtonColor: '#8E2D89' });
                                }
                            })
                            .catch(err => {
                                Swal.close();
                                Swal.fire({ title: '⚠️ Error de conexión', text: 'No se pudo conectar con el servidor. Por favor intenta de nuevo.', icon: 'error', confirmButtonText: 'Entendido', confirmButtonColor: '#8E2D89' });
                            });
                        }
                    });
                }
        ...
        if (next) {
            next.addEventListener('click', (ev) => {
                ...
                const match = ok && cok && p === c;
                if (!match) return;
                if (iusacellServices.tae === null) IusacellServices.loadTAE(p);
                if (iusacellServices.postpago === null) IusacellServices.loadPostpago(p);
        ...
        body.querySelector('#close-from-phones')?.addEventListener('click', () => { IusacellServices.closeOverlay(); });
        footer.addEventListener('click', onFooter);
        IusacellServices._h = { c1: onClick, c2: onInput, c3: onFooter, i1: direct, i2: direct };
        validate();
    }
}

IusacellServices.openOverlay = ({ title, bodyHTML, footerHTML }) => {
    if (window.ServiceModal) {
        window.ServiceModal.open({ title, bodyHTML, footerHTML });
    } else {
        // Fallback: crear overlay propio
        let root = document.getElementById('iusacell-kiosk-root');
        if (!root) {
            root = document.createElement('div');
            root.id = 'iusacell-kiosk-root';
            document.body.appendChild(root);
            root.innerHTML = `<div id="${id}-kiosk-wrapper">${bodyHTML}</div>`;
            try { IusacellServices._prev = { h: document.documentElement.style.overflow, b: document.body.style.overflow }; document.documentElement.style.overflow = 'hidden'; document.body.style.overflow = 'hidden'; } catch { }
        }
    }
};

IusacellServices.closeOverlay = () => {
    IusacellServices.detenerActualizacionSaldo();
    if (window.ServiceModal) {
        window.ServiceModal.close();
        try { if (IusacellServices._prev) { document.documentElement.style.overflow = IusacellServices._prev.h || ''; document.body.style.overflow = IusacellServices._prev.b || ''; IusacellServices._prev = null; } } catch { }
    } else {
        const root = document.getElementById('iusacell-kiosk-root');
        if (root) root.remove();
        try { if (IusacellServices._prev) { document.documentElement.style.overflow = IusacellServices._prev.h || ''; document.body.style.overflow = IusacellServices._prev.b || ''; IusacellServices._prev = null; } } catch { }
    }
};
